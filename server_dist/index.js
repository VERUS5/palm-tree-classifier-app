...
async function retrieveRAGContext(query, category, lang = "en", topK = 5) {
  const allChunks = category ? await db.select({
  ...
  }).from(chunks).innerJoin(documents, eq2(chunks.documentId, documents.id)).where(eq2(documents.category, category)) : await db.select({
    id: chunks.id,
    documentId: chunks.documentId,
    topic: chunks.topic,
    content: chunks.content,
    ...
    chunkIndex: chunks.chunkIndex,
    category: documents.category
  }).from(chunks).innerJoin(documents, eq2(chunks.documentId, documents.id));
...
async function getAllKnowledgeBase() {
  ...
  for (const doc of docs) {
    const docChunks = await db.select().from(chunks).where(eq2(chunks.documentId, doc.id));
